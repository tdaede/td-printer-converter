...
fn main() {
    ...
    if let Some(serial_port_name) = args.serial {
        // serial mode
        ...
        loop {
            ...
            let (covered_x, covered_y) = Cz8pc4::decode(&mut print_job_bytes.as_slice(), &mut img);
        ...
        img_cropped.save(args.output.expect("Output filename not provided")).unwrap();
    }
}
